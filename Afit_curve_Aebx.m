function [core,back,spectrum] = Afit_curve_Aebx(eprestart,eprewidth,epoststart,epostwidth,outcore,outback)
...
lspectrum(:,1) = spectrum(:,1);
lspectrum(:,2) = log(spectrum(:,2));
sumx = sum(lspectrum(efilter,1));
sumxy = sum(lspectrum(efilter,1) .* lspectrum(efilter,2));
sumy = sum(lspectrum(efilter,2));
sumxsq = sum(lspectrum(efilter,1).^2);
n=sum(efilter);

%Calculate A and R
d = sumxsq.*n - sumx.*sumx;
r =(sumxy.*n - sumx.*sumy)./d;
a = exp(((sumy.*sumxsq./d - sumx.*sumxy./d)));
fprintf(1,'\nFit coefficients I(E) = A*E^r\n');
fprintf(1, 'r = %g A = %g\n',r,a);

%Generate core and background curve data from A and R
back = spectrum;
back(:,2) = a.*exp(r.*spectrum(:,1));
core(:,1) = spectrum(:,1);
