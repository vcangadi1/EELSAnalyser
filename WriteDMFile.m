function WriteDMFile(filename,data,logfile)
...
celltags={'ImageList 2 ImageData Data'};
...
fb=fopen(filename,'w');
tic;
fwrite(fb,d,'*uint8');  % read the whole file as bytes
dt=toc;
fclose(fb);

%m=reshape(output{1},[xdim ydim zdim]);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    function x=GetData(ftype,num)
        ...
        switch ftype
            ...
            case 4  % ushort
                if write
                    d(p:p+num*2-1) = typecast(data(:),'uint8');
                    x = data(:);
                else
            ...
            case 5  % ulong
                if write
                    d(p:p+num*4-1) = typecast(data(:),'uint8');
                    x = data(:);
                else
            ...
            case 7  % double
                if write
                    d(p:p+num*8-1) = typecast(data(:),'uint8');
                    x = data(:);
                else
...
%                     case 3  % long
%                         x = cast(6,'int32');
%                         d(p:p+3) = typecast(swapbytes(x),'uint8');
%                     case 5  % ulong
%                          x = cast(6,'int32');
%                         d(p:p+3) = typecast(swapbytes(x),'uint8');
%                     case 11  % int64
...
%                     case 3  % long
%                          x = typecast(6,'uint64');
%                         d(p:p+7) = typecast(swapbytes(x),'uint8');
%                     case 5  % ulong
%                          x = typecast(6,'uint64');
%                         d(p:p+7) = typecast(swapbytes(x),'uint8');
%                     case 11  % int64
