function [m, dims, scales, units, origins]=ReadDMFile(filename,logfile)
...
celltags={
    ...
    'ImageList 2 ImageData Calibrations Dimension 4 Scale'
    'ImageList 2 ImageData Calibrations Dimension 4 Units'
    'ImageList 2 ImageData Calibrations Dimension 4 Origin'
    'ImageList 2 ImageTags EELS-Spectrometer Dispersion-(eV/ch)'};
...
dim3=output{10};         % no. of images in a stack
s3=output{11};
unit3=char(output{12});
origin3=output{13};

dim4=output{14};
s4=output{15};
unit4=char(output{16});
origin4=output{17};

dispersion=output{18};

if numel(dim3)<1 || any(dim3)<1
    dim3=1;
    dims = [dim1 dim2];
    scales = [s1 s2];
    units = {unit1 unit2};
