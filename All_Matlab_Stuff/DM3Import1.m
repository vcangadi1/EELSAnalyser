...
% scales.
    ...
    %mycode------------------------------------------------------------
    convangle = regexp( metadm3file,'Convergence semi-angle \(mrad\)%%%%' );
    if( ~ isempty( convangle))
        dm3struct.conv_angle_mrad = readTagData( convangle(1));
    end
    collangle = regexp( metadm3file,'Collection semi-angle \(mrad\)%%%%' );
    if( ~ isempty( collangle))
        dm3struct.coll_angle_mrad = readTagData( collangle(1));
    end
    %-------------------------------------------------------------------    
