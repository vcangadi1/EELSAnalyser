function RLucy(specFile, kernelFile, niter)
% 
% Details in R.F.Egerton: EELS in the Electron Microscope, 3rd edition, Springer 2011

    fprintf(1,'\n----------------RLucy---------------\n\n');
if(nargin < 3)
    fprintf('Alternate Usage: RLucy(''specFile'', ''kernelFile'', numIterations)\n\n');

    specFile = input('RLucy: Spectrum data file name (e.g. SpecGen.psd): ','s');
    kernelFile = input('Kernel file name (leave blank to generate from spectrum): ','s');   
    niter = input('Number of iterations (e.g. 15): ');
else
    fprintf('RLucy: Spectrum data file name: %s\n',specFile);
