...
figure;
hold on;
back = zeros(N,length(E));
ext = zeros(N,length(E)+1);
e = EELS_spectrum;
while(ne>=1)
    %back(:,ne) = [cf1(1:E(ne),ne); cf2(E(ne)+1:N,ne)];
    %back(:,ne) = [zeros(1,E(ne)).'; cf2(E(ne)+1:N,ne)];
    back(:,ne) = [EELS_spectrum(1:E(ne)); cf2(E(ne)+1:N,ne)];
    ext(:,ne) = EELS_spectrum - back(:,ne);
    ind(:,ne) = 
    ne = ne-1;
end

%plot(((1:N).*D +origin+offset).',back,'LineWidth',2);
plot(((1:N).*D +origin+offset).',ext,'LineWidth',2);
