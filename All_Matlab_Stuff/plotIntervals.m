...
for i = 1:numel(k) - 1
    ...
    text( minX, Y+2.5*W, sprintf('%d + %d', minR, maxR-minR) ,'FontWeight','bold');
end
set(gca, 'YTick', 1:max(id));
ylabel('Worker Number','FontWeight','bold');
xlabel('Time/s','FontWeight','bold');
title(sprintf('Mean work time: %d ms', ceil(mean(f-s)*24*3600*1000)),'FontWeight','bold');
figure(gcf)
