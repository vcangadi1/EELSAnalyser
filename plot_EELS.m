...
if((handles.EELS.SI_y*handles.EELS.step_size.y > 1000) && strcmp(handles.EELS.step_size.yunit,'nm'))
    xlabel(['Columns (',num2str(sprintf('%.2f',(handles.EELS.SI_y*handles.EELS.step_size.y)/1000)),'\mum)'],'FontWeight','bold','FontSize',14);
    plot_label_FOV(handles.axes1,(handles.EELS.SI_y*handles.EELS.step_size.y)/1000,'micro',handles.EELS.SI_x,handles.EELS.SI_y);
else
    xlabel(['Columns (',num2str(sprintf('%.2f',handles.EELS.SI_y*handles.EELS.step_size.y)),handles.EELS.step_size.yunit,')'],'FontWeight','bold','FontSize',14);
...
if((handles.EELS.SI_x*handles.EELS.step_size.x > 1000) && strcmp(handles.EELS.step_size.xunit,'nm'))
    ylabel(['Rows (',num2str(sprintf('%.2f',(handles.EELS.SI_x*handles.EELS.step_size.x)/1000)),'\mum)'],'FontWeight','bold','FontSize',14);
else
    ylabel(['Rows (',num2str(sprintf('%.2f',handles.EELS.SI_x*handles.EELS.step_size.x)),handles.EELS.step_size.xunit,')'],'FontWeight','bold','FontSize',14);
end
title(['Size = ',num2str(handles.EELS.SI_x),' x ',num2str(handles.EELS.SI_y),' pixel'],'FontWeight','bold','FontSize',14);
axis image
...
if(isfield(handles.EELS,'calibrated_energy_loss_axis'))
    plot(squeeze(handles.EELS.calibrated_energy_loss_axis(handles.r,handles.c,:)),...
        squeeze(handles.EELS.SImage(handles.r,handles.c,:)),...
        'LineWidth',2);
else
    plot(handles.EELS.energy_loss_axis,...
        squeeze(handles.EELS.SImage(handles.r,handles.c,:)),...
        'LineWidth',2);
end
...
xlabel('Energy-loss (eV) ');%,'FontWeight','bold','FontSize',14);
ylabel('Counts ');%,'FontWeight','bold','FontSize',14);
title(['Pixel = (',num2str(handles.r),',',num2str(handles.c),')']);%,'FontWeight','bold','FontSize',14);
grid on
grid minor
